...
pub async fn read_event() -> web3::Result<()> {
    ...
    let contract_address_str: String =
        env::var("CONTRACT_ADDRESS").expect("CONTRACT ADDRESS not found");

    let starting_block_str: String = env::var("STARTING_BLOCK").expect("STARTING BLOCK not found");
    ...
    let from_block =starting_block; // Starting block number
    let to_block = starting_block + block_range; // Add the block range from env
    let to_block_str = convert_to_string(to_block);
    let block_number = web3.eth().block_number().await?;

    println!("Latest Block Number: {:?}", block_number);
    // Get the current block number
    // let current_block: Latest = transport.get_block_number().await?;
    
    // // Convert current_block to string
    // let current_block_str = current_block.to_string();

    // println!("Current block number: {}", current_block_str);

    // let to_block_str  = to_block.to_string();

    println!("Block From {:?}", from_block);
    println!("Block To {:?}", to_block);
    // redis_connection::set_value("LAST_BLOCK", to_block_str).await.unwrap();
